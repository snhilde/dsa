...
// Sort the list using a hashing algorithm. The list must be a slice of a uniform data type.
// Note: The efficiency of this algorithm decreases as the range of possible values in the list increases.
func Hash(list interface{}) error {
	...
	// We now know that we have a slice of either numbers, booleans, or strings. Let's make sure it's not strings.
	v := at(0)
	k := v.Kind()
	if k == reflect.String {
		return errors.New("This algorithm does not support strings")
	}

	// Rebuild the slice with our common type.
	f := make([]int64, length)
	for i := 0; i < length; i++ {
		v :== at(i)
		switch k {
		...
		case reflect.Bool:
			f[i] = int64(v.Bool())
		default:
			// For this algorithm, we can only use values with (practically) finite ranges.
			return errors.New(fmt.Sprintf("Invalid underlying type (%s)", k))
		}
	}
	...
	for _, v := range f {
		...
		hash := int(math.Abs(float64(v % int64(length))))
	...
	index := 0
	for i := low; i <= high; i++ {
		hash := int(math.Abs(float64(i % int64(length))))
		for _, v := range table[hash] {
			if v == i {
				// TODO: how do you put the value back in to the list?
				list[index] = v
				index++
			}
		}
